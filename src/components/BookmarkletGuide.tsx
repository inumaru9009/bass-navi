...
const BOOKMARKLET_CODE = `javascript:(function(){var sheet=document.querySelector('.musical-sheet');if(!sheet){alert('U-FRETの曲ページで実行してください');return;}var title=document.querySelector('h1')?.textContent?.trim()||document.title.split('|')[0].trim();var artist=document.querySelector('.p-detail-cont__artist')?.textContent?.trim()||'';var lines=[];if(title)lines.push('曲名: '+title);if(artist)lines.push('アーティスト: '+artist);lines.push('---');sheet.querySelectorAll('.chord-row').forEach(function(row){var chords=[];var lyrics=[];row.querySelectorAll('.chord').forEach(function(chord){var rt=chord.querySelector('rt');var cols=chord.querySelectorAll('.mejiowvnz .col');var lyric=Array.from(cols).map(function(c){var t=(c.innerText||c.textContent);return t===''?' ':t;}).join('').replace(/[ \t\n\r]+/g,' ').trim();chords.push(rt?rt.textContent.trim():'');lyrics.push(lyric);});var chordLine=chords.filter(function(c){return c;}).join('  ');var lyricLine=lyrics.reduce(function(acc,lyric){if(!acc)return lyric;var lc=acc[acc.length-1];var fc=lyric[0];if(/[a-zA-Z!?,'.]/.test(lc)&&/[a-zA-Z(]/.test(fc)){return acc+' '+lyric;}return acc+lyric;},'');if(chordLine)lines.push(chordLine);if(lyricLine.trim())lines.push(lyricLine);});var text=lines.join('\\n');var capo=0;var capoEl=null;var directSels=['#capo','#chordCapo','#input-capo','select[name=\"capo\"]','select[id*=\"capo\"]','select[id*=\"Capo\"]'];for(var di=0;di<directSels.length;di++){capoEl=document.querySelector(directSels[di]);if(capoEl)break;}if(!capoEl){document.querySelectorAll('select').forEach(function(s){if(capoEl)return;var ctx=[(s.id||''),(s.name||''),(s.className||''),(s.previousElementSibling?s.previousElementSibling.textContent:''),(s.parentElement?s.parentElement.textContent:'')].join(' ');if(ctx.indexOf('カポ')>=0||ctx.toLowerCase().indexOf('capo')>=0){capoEl=s;}});}if(capoEl){capo=parseInt(capoEl.value,10)||0;}var inputCapo=prompt('カポ数を確認・入力してください（カポなしは0）:',capo);if(inputCapo===null)return;capo=parseInt(inputCapo,10)||0;var naviUrl='https://bass-navi.vercel.app/'+(capo>0?'?capo='+capo:'');navigator.clipboard.writeText(text).then(function(){alert('譜面をコピーしました！\\nBass Naviに貼り付けてください。');window.open(naviUrl,'_blank');}).catch(function(){prompt('以下をコピーしてBass Naviに貼り付けてください:',text);window.open(naviUrl,'_blank');});})();`;

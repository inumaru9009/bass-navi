...
- 使われているスケール
- メインのコード進行パターン名（王道進行・カノン進行など）
- ノンダイアトニックコードがあれば言及
- ベース初心者向けの一言

## 譜面のフォーマットについて（重要）
この譜面はu-fretの形式で、以下のペア構造になっている：
  [コード行] ← そのコードで演奏する
  [歌詞行]   ← そのコードに乗せて歌う歌詞

コード行は必ず直後の歌詞行とセットで同じセクションに属する。
「コード行のみが続いているからイントロ」とは判断しないこと。

## イントロの判定基準
イントロ（intro）と判定するのは以下の条件を両方満たす場合のみ：
1. 曲の冒頭に位置する
2. そのコード行の直後の行も歌詞ではなく、コード行または空行である

直後の行に歌詞が存在するコード行は、たとえ曲の冒頭付近にあってもイントロではなくverseに含める。
